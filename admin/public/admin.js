...
async function loadOverview() {
  try {
    ...
    setTimeout(() => initAdminOverviewMiniMap(), 250);
    ...
    body.innerHTML = `
      <div style="display:grid;grid-template-columns:2fr 1fr;gap:28px">
        <div>
          ...
          <div style="background:#fafbfc;border:1.5px solid var(--gray-200);border-radius:14px;padding:16px 18px;margin-bottom:20px;box-shadow:var(--shadow-sm)">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
              <div style="font-size:13.5px;font-weight:800;color:var(--navy);display:flex;align-items:center;gap:6px">
                <span>📈</span> Project Lifecycle Timeline
              </div>
              <span class="badge badge-navy" style="font-size:11px;font-weight:750">Active Stage: ${(c.status||'submitted').replace(/_/g,' ').toUpperCase()}</span>
            </div>
            ${renderAdminTimelineTracker(c)}
          </div>

          <!-- Citizen Ground Problem Statement -->
          <div style="background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:14px;padding:16px 18px;margin-bottom:18px">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
              <div style="font-size:12.5px;font-weight:800;color:#166534;display:flex;align-items:center;gap:6px">
                <span>📝</span> Citizen Ground Problem Statement
              </div>
              <span style="font-size:10.5px;font-weight:750;color:#15803D;background:#DCFCE7;padding:2px 8px;border-radius:999px">Submitted by Resident</span>
            </div>
            <p style="font-size:13.5px;color:#14532D;line-height:1.7;margin:0;white-space:pre-wrap;font-weight:500">${c.description || 'No detailed description provided by citizen.'}</p>
          </div>
          ...
          <!-- Citizen Field Evidence Gallery (Photos & Video) -->
          <div style="background:#F8FAFC;border:1.5px solid #E2E8F0;border-radius:14px;padding:16px 18px;margin-bottom:18px">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
              <div style="font-size:13px;font-weight:800;color:var(--navy);display:flex;align-items:center;gap:6px">
                <span>📸</span> Citizen Field Evidence (${displayPhotos.length} Photos${citizenVideos.length > 0 ? ', 1 Video' : ''})
              </div>
              <span style="font-size:10.5px;font-weight:750;color:#059669;background:#ECFDF5;padding:2px 8px;border-radius:999px">● Ground Verified</span>
            </div>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(130px, 1fr));gap:12px;margin-bottom:14px">
              ${displayPhotos.map((p, idx) => `
                <div style="border-radius:10px;overflow:hidden;border:1.5px solid #CBD5E1;height:120px;position:relative;background:#E2E8F0;cursor:pointer" onclick="window.open('${p.url}', '_blank')">
                  <img src="${p.url}" style="width:100%;height:100%;object-fit:cover" alt="Citizen Photo ${idx + 1}" onerror="this.src='${fallbackPhoto}'" />
                  <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent, rgba(0,0,0,0.8));color:white;font-size:9.5px;padding:3px 6px;font-weight:700">
                    Photo ${idx + 1}
                  </div>
                </div>
              `).join('')}
            ...
            <!-- Citizen Video or Permanent Pill -->
            <div>
              ${citizenVideos.length > 0 ? `
                <div style="border-radius:12px;overflow:hidden;border:1.5px solid #CBD5E1;background:#0F172A;position:relative">
                  <video src="${citizenVideos[0].url}" controls style="width:100%;max-height:220px;object-fit:contain;background:#000"></video>
                  <div style="position:absolute;top:8px;left:8px;background:rgba(220,38,38,0.9);color:white;font-size:9px;font-weight:800;padding:2px 6px;border-radius:4px">
                    ● CITIZEN FIELD VIDEO
                  </div>
                </div>
              ` : `
                <div style="border-radius:10px;border:1.5px dashed #CBD5E1;background:#FFFFFF;padding:16px;text-align:center;display:flex;align-items:center;justify-content:center;gap:8px">
                  <span style="font-size:16px;color:#94A3B8">📹</span>
                  <span style="font-size:12px;font-weight:700;color:#64748B">No video uploaded by submitter or citizen</span>
                </div>
              `}
            </div>
          </div>
        ...
        <div>
          ...
          ${c.assignedUniversity?`
            ...
            <div style="padding:12px;background:#FFFBEB;border:1.5px solid #FCD34D;border-radius:12px;margin-bottom:12px">
              <div style="font-size:11px;font-weight:800;color:#92400E;margin-bottom:4px">STATUS: PENDING VALIDATION</div>
              <div style="font-size:12px;color:#B45309;line-height:1.4">
                Citizen report is unassigned. Review and validate before university allocation.
              </div>
            </div>`}
          ...
          <div style="margin-top:16px;border-top:1px solid var(--gray-100);padding-top:14px">
            <div style="font-size:12px;font-weight:800;color:var(--gray-400);letter-spacing:0.6px;text-transform:uppercase;margin-bottom:10px">Decision & Audit History</div>
            ${(c.statusHistory && c.statusHistory.length > 0) ? c.statusHistory.map(h => `
              <div class="decision-item" style="padding:8px 0;border-bottom:1px solid var(--gray-100)">
                <div class="di-dot" style="${h.status==='validated'?'background:#059669':h.status==='rejected'?'background:#dc2626':''}"></div>
                <div>
                  <div class="di-date">${Utils.formatDate(h.changedAt, true)} · <strong>${h.changedBy?.name || 'JanSetu Authority'}</strong></div>
                  <div class="di-action" style="font-weight:700;color:var(--gray-900)">Status → ${h.status.replace(/_/g,' ').toUpperCase()}</div>
                  ${h.note ? `<div style="font-size:12px;color:var(--gray-600);margin-top:2px;font-style:italic">"${h.note}"</div>` : ''}
                </div>
              </div>`).join('') : `
              <div class="decision-item" style="padding:10px 0">
...
function initHeatmap() {
  ...
  if (!el) return;
  if (window.PanIndiaHeatmap) {
    if (!window.panIndiaMapInstance) {
      window.panIndiaMapInstance = new PanIndiaHeatmap({
        containerId: 'jharkhand-heatmap',
        panelId: 'districtDetailPanel',
        selectId: 'adminStateSelect',
        categorySelectId: 'adminMapCategoryFilter',
        role: 'admin'
      });
    } else {
      window.panIndiaMapInstance.invalidateSize();
    }
  }
}
...
function renderAdminTimelineTracker(challengeOrStatus) {
  ...
  let html = '<div class="timeline-milestones-row" style="display:flex;align-items:flex-start;position:relative;width:100%;margin:12px 0 6px;padding:0 2px">';
  STAGES.forEach((st, idx) => {
    ...
    html += `
      <div class="timeline-step-item" style="display:flex;flex-direction:column;align-items:center;flex:1;text-align:center;position:relative;min-width:65px">
        <div class="timeline-dot-bubble ${bubbleClass}">
          ${bubbleContent}
        </div>
        <div class="timeline-step-label ${isActive ? 'active' : ''}" style="font-size:11px;font-weight:${isActive ? '800' : '700'};color:${isActive ? '#002D62' : '#334155'};margin-top:6px;line-height:1.2">
          ${st.name}
        </div>
        <div class="timeline-step-date" style="font-size:9.5px;font-weight:750;color:#ea580c;margin-top:2px">
          ${st.date}
        </div>
        <div class="timeline-step-note" style="font-size:9px;color:#64748b;margin-top:2px;max-width:85px">
          ${st.note}
        </div>
      </div>
    `;
    ...
    if (idx < STAGES.length - 1) {
      ...
      html += `
        <div class="timeline-connecting-line ${isLineActive ? 'active-line' : ''}" style="flex:1;height:3px;background:${isLineActive ? '#16A34A' : '#e2e8f0'};margin-top:14px;z-index:1;border-radius:2px"></div>
      `;
    }
  });
